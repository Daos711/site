...
/**
 * The embedded document uses `min-height: 100vh` on its body and app root. That is useful when
 * it runs by itself, but inside an auto-sized iframe it makes the old iframe height look like
 * content and prevents the frame from shrinking after a screen change. This runtime-only
 * override keeps the bundle untouched while making its natural content height measurable.
 */
const AUTO_HEIGHT_STYLES = `
  html, body, #app {
...
function documentContentHeight(document: Document): number {
  ...
  for (const child of body.children) {
    ...
    const marginBottom = view
      ? Number.parseFloat(view.getComputedStyle(child).marginBottom) || 0
      : 0;
    contentHeight = Math.max(
      contentHeight,
      rect.top - bodyTop + Math.max(rect.height, child.scrollHeight) + marginBottom,
    );
  }

  // Root scroll heights are never smaller than the iframe viewport. Only use them when they
  // describe real overflow; otherwise a previously tall frame could never shrink again.
...
function ElkaFrame({ src }: { src: string }) {
  ...
  const fallbackHeight = `max(${FRAME.minHeight}px, calc(100dvh - ${FRAME.reserved}))`;
  ...
  useEffect(() => {
    ...
    function connectDocument() {
      ...
      try {
        ...
        // The production embed is same-origin. If that architecture changes, leave the usable
        // fallback height and its normal scrolling in place instead of clipping the document.
        return;
      }
      ...
      sizingStyles.dataset.elkaFrameSizing = "";
      sizingStyles.textContent = AUTO_HEIGHT_STYLES;
      (document.head ?? document.documentElement).appendChild(sizingStyles);

      let animationFrame = 0;
      let settleFrame = 0;
      let probing = false;
      let viewportDriven = false;
      let previousFrameHeight = 0;
      let previousContentHeight = 0;

      const updateHeight = () => {
        animationFrame = 0;
        if (probing || viewportDriven) return;

        const currentFrameHeight = frame.clientHeight;
        const nextHeight = Math.max(FRAME.minHeight, documentContentHeight(document));

        if (previousFrameHeight && currentFrameHeight !== previousFrameHeight) {
          const frameDelta = currentFrameHeight - previousFrameHeight;
          const contentDelta = nextHeight - previousContentHeight;

          // The battle canvas deliberately follows `window.innerHeight`. If content and frame
          // grow one-for-one, applying the new content height again would create an endless
          // resize staircase. Return that viewport-driven state to the stable fallback height.
          if (frameDelta > 0 && Math.abs(contentDelta / frameDelta - 1) < 0.02) {
            viewportDriven = true;
            probeHeight();
            return;
          }
        ...
        previousContentHeight = nextHeight;
        if (Math.abs(nextHeight - currentFrameHeight) > 1) {
          frame.style.height = `${nextHeight}px`;
        }
      };
      const scheduleHeightUpdate = () => {
        if (!probing && !animationFrame) {
          animationFrame = window.requestAnimationFrame(updateHeight);
        }
      };

      // Re-measure screen changes and outer viewport changes from the same stable baseline.
      // This lets a long start screen shrink back to a viewport-sized battle without treating
      // the battle canvas's own `innerHeight` sizing as more document content.
      const probeHeight = () => {
        const keepViewportDriven = viewportDriven;
        probing = true;
        ...
        settleFrame = window.requestAnimationFrame(() => {
          settleFrame = window.requestAnimationFrame(() => {
            ...
            const nextHeight = Math.max(
              FRAME.minHeight,
              documentContentHeight(document),
            );
            previousFrameHeight = currentFrameHeight;
            previousContentHeight = nextHeight;
            if (!keepViewportDriven && Math.abs(nextHeight - currentFrameHeight) > 1) {
              frame.style.height = `${nextHeight}px`;
            }
            probing = false;
            if (!keepViewportDriven) scheduleHeightUpdate();
          });
        });
      };
      const probeScreenHeight = () => {
      ...
      const screenObserver = new MutationObserver(probeScreenHeight);
      for (const root of document.body.children) {
        for (const screen of root.children) {
      ...
      contentWindow.addEventListener("resize", scheduleHeightUpdate);
      window.addEventListener("resize", probeHeight);
      window.visualViewport?.addEventListener("resize", probeHeight);
      probeHeight();
      ...
      disconnectDocument = () => {
        ...
        contentWindow.removeEventListener("resize", scheduleHeightUpdate);
        window.removeEventListener("resize", probeHeight);
        window.visualViewport?.removeEventListener("resize", probeHeight);
        sizingStyles.remove();
      };
    }

    frame.addEventListener("load", connectDocument);
    if (frame.contentDocument?.readyState === "complete") connectDocument();
