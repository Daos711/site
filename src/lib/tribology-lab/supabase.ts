...
export const getPlayerId = _getPlayerId;
...
// Теперь используется getAnonymousPlayerId из общего модуля
...
export interface PendingTribolabResult {
...
const PENDING_RESULT_KEY = 'tribolab_pending_result';
const PENDING_RESULT_MAX_AGE = 5 * 60 * 1000; // 5 минут

export function savePendingResult(result: Omit<PendingTribolabResult, 'timestamp'>): void {
  if (typeof window === 'undefined') return;
  const data: PendingTribolabResult = {
    ...result,
    timestamp: Date.now(),
  };
...
export function getPendingResult(): PendingTribolabResult | null {
  if (typeof window === 'undefined') return null;
  const raw = localStorage.getItem(PENDING_RESULT_KEY);
  if (!raw) return null;

  try {
    const data: PendingTribolabResult = JSON.parse(raw);
